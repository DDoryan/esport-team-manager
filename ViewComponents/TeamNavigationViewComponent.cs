...
public sealed class TeamNavigationViewComponent : ViewComponent
{
    ...
    public async Task<IViewComponentResult> InvokeAsync()
    {
        ...
        IReadOnlyCollection<TeamNavigationItemViewModel> navigationTeams = teams
            ...
            .ToArray();
        Guid? activeTeamId = GetActiveTeamId();
        TeamNavigationItemViewModel? activeTeam = activeTeamId.HasValue
            ? navigationTeams.SingleOrDefault(team => team.TeamId == activeTeamId.Value)
            : null;
        ...
        bool managementIsActive = activeTeam is not null
            && string.Equals(currentController, "Teams", StringComparison.OrdinalIgnoreCase)
            && string.Equals(currentAction, "Management", StringComparison.OrdinalIgnoreCase);
